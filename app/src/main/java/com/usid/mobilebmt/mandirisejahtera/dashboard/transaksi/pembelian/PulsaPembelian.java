...
public class PulsaPembelian extends AppCompatActivity implements IAsyncHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pulsa_pembelian);
        cnd = new CtdPPOB(PulsaPembelian.this);
        ...
        SnHp telpMan = new SnHp(PulsaPembelian.this);
        ...
        btnOK.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (jwtlocal.equals("0")) {
                    Toast.makeText(PulsaPembelian.this, "Tidak bisa diproses, periksa koneksi anda tunggu hingga indikator berwarna biru", Toast.LENGTH_SHORT).show();
                } else {
                    if (edOperator.getText().toString().equals("") || edNominal.getText().toString().equals(""))
                        showAlert("KESALAHAN", "Penyedia layanan dan nominal tidak boleh kosong!");
                    else if (edNoHp.getText().toString().equals(""))
                        showAlert("KESALAHAN", "Nomor Handphone tidak boleh kosong!");
                        ...
                        String strNamaOperator = Utility.namaOperator(edNoHp.getText().toString().trim());
                        if (strNamaOperator.equals(edOperator.getText().toString().trim())) {
                            new AsyncInqBMT().execute();
                        } else {
                            edNoHp.setText(edNoHp.getText().toString().substring(0, 4));
                            edNoHp.setSelection(edNoHp.getText().toString().length());
                            showAlert("KESALAHAN", "Nomor handphone tidak sesuai dengan penyedia layanan!");
                        }
                    }
                }
            }
        ...
        btnDaftarTrf.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(PulsaPembelian.this, DaftarPulsa.class);
                startActivityForResult(intent, 0);
            }
        });
        pdLoadingInq = new ProgressDialog(PulsaPembelian.this);
        ...
        switch (view.getId()) {
            ...
            case R.id.edNom:
                if (edOperator.getText().toString().equals("")) {
                    showAlert("KESALAHAN", "Silahkan pilih penyedia layanan terlebih dahulu!");
                } else {
    ...
    private class AsyncInqListPulsa extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL objs = new URL(MyVal.URL_BASE_PPOB() + new NumSky(PulsaPembelian.this).decrypt(getResources().getString(R.string.urlListPulsa)));
    ...
    private class AsyncListProduk extends AsyncTask<Void, Void, Void> {
        ProgressDialog pdLoading = new ProgressDialog(PulsaPembelian.this);
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE() + new NumSky(PulsaPembelian.this).decrypt(getResources().getString(R.string.urllistproduk)));
        ...
        protected void onPostExecute(Void result) {
            ...
            if (stInfoMutasi) {
                ListAdapter adapter = new SimpleAdapter(PulsaPembelian.this, oslist, R.layout.list_item, new String[]{"nmproduk"}, new int[]{R.id.text1});
                AlertDialog.Builder alertDialog = new AlertDialog.Builder(PulsaPembelian.this);
                list = new ListView(PulsaPembelian.this);
    ...
    private class AsyncInqBMT extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE() + new NumSky(PulsaPembelian.this).decrypt(getResources().getString(R.string.urlinquirypulsa)));
                HttpURLConnection conJ = (HttpURLConnection) obj.openConnection();
                conJ.setRequestMethod("POST");
                conJ.setRequestProperty("Content-Type", "application/json");
                conJ.setRequestProperty("Authorization", getPrefsAuthToken());
                String strCek = Utility.toJsonString(Utility.md5(strNokartu), Utility.md5(strImsi), "0", strPRDID, "0", 0, "0", "0", "0", "0", "0", versi);
                conJ.setConnectTimeout(30000);
                conJ.setReadTimeout(29000);
                conJ.setDoOutput(true);
                DataOutputStream wr = new DataOutputStream(conJ.getOutputStream());
    ...
    private class AsyncINQPPOB extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE_PPOB() + new NumSky(PulsaPembelian.this).decrypt(getResources().getString(R.string.urlCekStatusProd)));
                HttpURLConnection conJ = (HttpURLConnection) obj.openConnection();
                conJ.setRequestMethod("POST");
                conJ.setRequestProperty("Content-Type", "application/json");
                String strCek = Utility.toJsonStringPPOB(strPPOBID, strKeyPass, strPRDIDBiller, "", "");
                conJ.setConnectTimeout(30000);
                conJ.setReadTimeout(29000);
                conJ.setDoOutput(true);
                DataOutputStream wr = new DataOutputStream(conJ.getOutputStream());
    ...
    private class AsyncPaymentPulsaBMT extends AsyncTask<Void, Void, Void> {
        ProgressDialog pdLoading = new ProgressDialog(PulsaPembelian.this);
        private Boolean status = false;
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE_PPOB() + new NumSky(PulsaPembelian.this).decrypt(getResources().getString(R.string.urlPulsa)));
                HttpURLConnection conJ = (HttpURLConnection) obj.openConnection();
                conJ.setRequestMethod("POST");
                conJ.setRequestProperty("Content-Type", "application/json");
...
//                String strCek = toJsonString(Utility.md5(strNokartu), Utility.md5(strImsi), Utility.md5(strPIN), JenisTRX, strIDPEL + " a.n " + strNama, longJml, trxID, strCAID, md5(strKeyPass), strIDREFF,strIDPEL, Long.toString(longJml);
                String strCek = Utility.toJsonString(Utility.md5(strNokartu), Utility.md5(strImsi), Utility.md5(strPIN), strPRDIDBiller, strIDPEL, jumlah, trxID, strPPOBID, strKeyPass, "", strIDPEL, 0);
                conJ.setConnectTimeout(30000);
                conJ.setReadTimeout(29000);
                conJ.setDoOutput(true);
                DataOutputStream wr = new DataOutputStream(conJ.getOutputStream());
...
//                    String keta = "Transaksi Pembelian Pulsa Berhasil" + "\nTanggal    : " + strDate + "\nProduk     : " + layanan + "\nNomor HP.  : " + strIDPEL + "\nNominal    : " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(nominal), 15) + "\nBiaya Admin: " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(biayaAdmin), 15) + "\nTotal Bayar: " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(jumlah), 15) + "\nSN " + struk + "\n";
                    String keta = "Transaksi Pembelian Pulsa Berhasil" + "\nTanggal    : " + strDate + "\nFaktur     : " + strFaktur + "\nTrxID      : " + trxID + "\nDari       : " + strRekAsal + "\nProduk     : " + layanan + "\nNomor HP.  : " + strIDPEL + "\nNominal    : " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(nominal), 15) + "\nBiaya Admin: " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(biayaAdmin), 15) + "\nTotal Bayar: " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(jumlah), 15) + "\nSN " + struk + "\n";
                    SysDB dbsys = new SysDB(PulsaPembelian.this);
                    dbsys.open();
...
//                    dbsys.insertSys(strTgl, "Pembelian Pulsa Seluler (Reff)\n" + keta);
                    dbsys.insertSys(strTgl, "Pembelian Pulsa Seluler SUKSES (Reff)\n" + keta);
                    dbsys.close();
                    showConfirmBeliSukses("PEMBELIAN BERHASIL", ""
                                    + strDate, "" + layanan,
                            "" + edNoHp.getText().toString(), "Rp "
                                    + Utility.DoubleToCurrency(nominal),
                            strBiaya.replace(": Rp. ", "Rp "),
                            "Rp " + Utility.DoubleToCurrency(jumlah), ""
                                    + struk);
                } else {
                    String msga = "#" + rc + " - " + ket + "\n";
                    SysDB dbsys = new SysDB(PulsaPembelian.this);
                    dbsys.open();
                    dbsys.CreateTableSys();
                    dbsys.insertSys(strTgl, "Pembelian Pulsa Seluler STATUS " + msga);
                    dbsys.close();
                    sdialog = true;
                    showExt("Pembelian Pulsa Seluler STATUS", msga);
                }
            } else {
                if (ket.startsWith("404")) {
                    ...
                    dbsys.open();
                    dbsys.CreateTableSys();
                    dbsys.insertSys(strTgl, "Pembelian Pulsa Seluler TIMEOUT " + msga);
                    dbsys.close();
                    sdialog = true;
                    showExt("Pembelian Pulsa Seluler TIMEOUT", msga);
                } else {
                    String msga = "#" + ket + "\n";
                    SysDB dbsys = new SysDB(PulsaPembelian.this);
                    dbsys.open();
                    dbsys.CreateTableSys();
                    dbsys.insertSys(strTgl, "Pembelian Pulsa Seluler GAGAL " + msga);
                    dbsys.close();
                    sdialog = true;
                    showExt("Pembelian Pulsa Seluler GAGAL", msga);
                }
    ...
    private void AlertDialogViewOperator() {
        AlertDialog.Builder builder = new AlertDialog.Builder(PulsaPembelian.this);
    ...
    private void showExt(String title, String message) {
        new AlertDialog.Builder(PulsaPembelian.this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                finish();
                dialog.dismiss();
    ...
    private void showConfirmBeli(String Judul, final String prod, final String nohp, String nomi, String admin, String jumlah) {
        final Dialog dialog = new Dialog(PulsaPembelian.this);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.custom_dialog_confirm_trxpulsa);
        ...
        int jumla = 0;
        Cursor cur1 = dbsys.cekPulsaData(edNoHp.getText().toString());
        if (cur1.moveToFirst()) {
            jumla = cur1.getInt(0);
        }
        cur1.close();
                ...
                if (jwtlocal.equals("0"))
                    ...
                    if (cbsimpan.isChecked()) {
                        if (edNama.getText().toString().equals("")) {
                            Toast.makeText(PulsaPembelian.this, "Tidak bisa diproses, Nama tidak boleh kosong", Toast.LENGTH_SHORT).show();
                        } else {
                            try {
                                SysDB dbsys = new SysDB(PulsaPembelian.this);
                                dbsys.open();
                                String info = edNama.getText().toString().trim();
                                dbsys.insertPls(edNoHp.getText().toString(), info);
                                dbsys.close();
            ...
            public void onClick(View v) {
                ...
                if (jwtlocal.equals("0") || timeup)
                    ...
                    if (Utility.is6digit(strPIN)) {
                        new AsyncPaymentPulsaBMT().execute();
                    } else {
    ...
    private void showConfirmBeliSukses(String Judul, String tgls, String prod, String nohp, String nomi, String admin, String jumlah, String sn) {
        ...
        Intent antarbank = new Intent(PulsaPembelian.this, PembelianSukses.class);
        ...
        antarbank.putExtra("judul", "Pulsa Pembelian Berhasil");
        ...
        /*final Dialog dialog = new Dialog(PulsaPembelian.this);
        Utility.playNotificationSound(PulsaPembelian.this);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.custom_dialog_confirm_trxpulsasukses);
        TextView textPin = (TextView) dialog.findViewById(R.id.textPin);
        textPin.setText(Judul);
...
//            View v1 = getWindow().getDecorView().getRootView();
            ...
            Toast.makeText(PulsaPembelian.this, "Gagal menyimpan screenshoot", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
    ...
    protected void onResume() {
        ...
        if (stats && sts > 1) {
            ...
            AuthLoginPPOB task = new AuthLoginPPOB(PulsaPembelian.this, PulsaPembelian.this);
            task.execute();
        }
    }
