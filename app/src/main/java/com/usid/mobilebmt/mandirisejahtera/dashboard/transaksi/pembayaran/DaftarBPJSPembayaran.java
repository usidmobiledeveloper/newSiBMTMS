...
public class DaftarBPJSPembayaran extends AppCompatActivity {
    private SysDB dbsys;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbsys = new SysDB(DaftarBPJSPembayaran.this);
        dbsys.open();
        dbsys.CreateTableBPJSpasca();
        int jumla = 0;
        Cursor cur1 = dbsys.cekBPJSpascaData();
        if (cur1.moveToFirst()) {
            jumla = cur1.getInt(0);
        }
        cur1.close();
        ...
        rekening.clear();
        nama.clear();
        Cursor cur = dbsys.daftarBPJSpasca();
        if (cur.moveToFirst()) {
            do {
    ...
    private void showAlert(String title, String message, final String rek) {
        new AlertDialog.Builder(this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("YA", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                SysDB dbsys = new SysDB(DaftarBPJSPembayaran.this);
                dbsys.open();
                dbsys.DeleteBPJSpasca(rek);
                dbsys.close();
                finish();
                dialog.dismiss();
            }
