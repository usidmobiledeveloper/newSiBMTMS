...
public class DaftarPLNPembayaran extends AppCompatActivity {
    private SysDB dbsys;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbsys = new SysDB(DaftarPLNPembayaran.this);
        dbsys.open();
        dbsys.CreateTablePLNpasca();
        int jumla = 0;
        Cursor cur1 = dbsys.cekPLNpascaData();
        if (cur1.moveToFirst()) {
            jumla = cur1.getInt(0);
        }
        cur1.close();
        ...
        rekening.clear();
        nama.clear();
        Cursor cur = dbsys.daftarPLNpasca();
        if (cur.moveToFirst()) {
            do {
    ...
    private void showAlert(String title, String message, final String rek) {
        new AlertDialog.Builder(this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("YA", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                SysDB dbsys = new SysDB(DaftarPLNPembayaran.this);
                dbsys.open();
                dbsys.DeletePLNpasca(rek);
                dbsys.close();
                finish();
                dialog.dismiss();
            }
