...
public class DaftarTELKOMPembayaran extends AppCompatActivity {
    private SysDB dbsys;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbsys = new SysDB(DaftarTELKOMPembayaran.this);
        dbsys.open();
        dbsys.CreateTableTELKOM();
        int jumla = 0;
        Cursor cur1 = dbsys.cekTELKOMData();
        if (cur1.moveToFirst()) {
            jumla = cur1.getInt(0);
        }
        cur1.close();
    ...
    private void showAlert(String title, String message, final String rek) {
        new AlertDialog.Builder(this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("YA", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                SysDB dbsys = new SysDB(DaftarTELKOMPembayaran.this);
                dbsys.open();
                dbsys.DeleteTELKOM(rek);
                dbsys.close();
                finish();
                dialog.dismiss();
            }
