...
import com.usid.mobilebmt.mandirisejahtera.databinding.ActivityTelkomPembayaranSuksesBinding;
...
public class TelkomPembayaranSukses extends AppCompatActivity {
    ...
    ActivityTelkomPembayaranSuksesBinding binding;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding=ActivityTelkomPembayaranSuksesBinding.inflate(getLayoutInflater());
        ...
        Utility.playNotificationSound(TelkomPembayaranSukses.this);
        SnHp telpMan = new SnHp(TelkomPembayaranSukses.this);
        if (5 != telpMan.telephonyManager().getSimState()) {
            strImsi = "TIDAK ADA KARTU";
        } else {

                strImsi = Utility.getIMSIRead(TelkomPembayaranSukses.this);
        ...
        if (strImsi.equals("TIDAK ADA KARTU")) {
            ...
            Utility.addRow(arr, TelkomPembayaranSukses.this, mKonten);
    ...
    private void takeScreenshot() {
        try {
            ...
            Toast.makeText(TelkomPembayaranSukses.this, "Gagal menyimpan screenshoot", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
    ...
    private void showExt(String title, String message) {
        new AlertDialog.Builder(TelkomPembayaranSukses.this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
...
//        Uri uri = Uri.parse("file://" + imageFile.getAbsolutePath());
        Uri uri = FileProvider.getUriForFile(TelkomPembayaranSukses.this, BuildConfig.APPLICATION_ID + ".provider", imageFile);
