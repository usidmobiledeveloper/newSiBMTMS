...
public class MultifinancePembayaran extends AppCompatActivity implements IAsyncHandler {
    private SharedPreferences config;
    private TextView output = null;
    private Bundle extras;
    private Button btnProsesMultifinance, btnDaftarTrf;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_multifinance_pembayaran);
        ButterKnife.bind(this);

        ActionBar actionBar = getSupportActionBar();
        //tambahan ayik==
        cnd = new CtdPPOB(MultifinancePembayaran.this);
        if (actionBar != null) {
        ...
        btnProsesMultifinance = (Button) findViewById(R.id.btnProses);
        ...
        btnProsesMultifinance.setVisibility(View.GONE);
        mKonten.setVisibility(View.GONE);
        config = getApplicationContext().getSharedPreferences("config", 0);
        SnHp telpMan = new SnHp(MultifinancePembayaran.this);
        if (5 != telpMan.telephonyManager().getSimState()) {
            ...
            /*try {
                ...
                strImsi = Utility.getIMSIRead(MultifinancePembayaran.this);
//            }
        }
        ...
        pdLoadingInq = new ProgressDialog(MultifinancePembayaran.this);
        ...
        btnProsesMultifinance.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (jwtlocal.equals("0")) {
                    Toast.makeText(MultifinancePembayaran.this, "Tidak bisa diproses, periksa koneksi anda tunggu hingga indikator berwarna biru", Toast.LENGTH_SHORT).show();
                } else {
                    showConfirmPinMultifinancePasca(getString(R.string.pin_mobilebmt));
                }
            }
        });
        btnDaftarTrf.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(MultifinancePembayaran.this, DaftarMultifinancePembayaran.class);
            ...
            public boolean onEditorAction(TextView view, int actionId, KeyEvent event) {
                if (actionId == EditorInfo.IME_ACTION_NEXT || actionId == EditorInfo.IME_ACTION_DONE) {
                    if (jwtlocal.equals("0")) {
                        ...
                        finis = false;
                        new AsyncInquiryMultifinancePPOB().execute();
                    }
                    return true;
                } else {
                    return false;
    ...
    private class AsyncInqBMT extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE() + new NumSky(MultifinancePembayaran.this).decrypt(getResources().getString(R.string.urlinquiry)));
...
//                    strCAID = (String) jsonObject.get("ppobupline");
    ...
    private class AsyncInquiryMultifinancePPOB extends AsyncTask<Void, Void, Void> {
        ProgressDialog pdLoading = new ProgressDialog(MultifinancePembayaran.this);
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE_PPOB() + new NumSky(MultifinancePembayaran.this).decrypt(getResources().getString(R.string.urlInqPPOB)));
                HttpURLConnection conJ = (HttpURLConnection) obj.openConnection();
                conJ.setRequestMethod("POST");
                conJ.setRequestProperty("Content-Type", "application/json");
//                String strCek = toJsonStringPPOB(strCAID, strKeyPass, JenisTRX, strIDPEL);
        ...
        protected void onPostExecute(Void result) {
            ...
            if (status) {
                ...
                if (rc.equals("0000")) {
                    ...
                    btnProsesMultifinance.setVisibility(View.VISIBLE);
                    ...
                    Utility.addRow(arr, MultifinancePembayaran.this, mKonten);
                } else {
                    edIdPelNo.requestFocus();
                    btnProsesMultifinance.setVisibility(View.GONE);
                    mKonten.setVisibility(View.GONE);
    ...
    private class AsyncPaymentMultifinancePascaBMT extends AsyncTask<Void, Void, Void> {
        ProgressDialog pdLoading = new ProgressDialog(MultifinancePembayaran.this);
        private Boolean status = false;
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE_PPOB() + new NumSky(MultifinancePembayaran.this).decrypt(getResources().getString(R.string.urlPayPPOB)));
                HttpURLConnection conJ = (HttpURLConnection) obj.openConnection();
                conJ.setRequestMethod("POST");
                conJ.setRequestProperty("Content-Type", "application/json");
...
//                    ket = strWiil + " Pascabayar" + "\nTanggal     : " + strDate + "\nFaktur      : " + strFaktur + "\nDari        : " + strRekAsal + "\nID Pelanggan: " + strIDPEL + "\nNama Pel    : " + strNama + "\nTotal Bayar : Rp. " + Utility.DoubleToCurrency(longJml) + "\nTrxID       : " + trxID + "\n";
                    ...
                    SysDB dbsys = new SysDB(MultifinancePembayaran.this);
                    dbsys.open();
                    dbsys.CreateTableSys();
                    dbsys.CreateTableMultifinance();
...
//                    dbsys.insertSys(strTgl, "Pembayaran " + strWiil + " (Reff)\n" + keta);
                    ...
                    int jumla = 0;
                    Cursor cur1 = dbsys.cekMultifinanceData(strIDPEL);
                    if (cur1.moveToFirst()) {
                        jumla = cur1.getInt(0);
                    }
                    cur1.close();
                    if (jumla == 0) dbsys.insertMultifinance(strIDPEL, strNama);
                    dbsys.close();
                    Intent intentPayment = new Intent(MultifinancePembayaran.this, MultifinancePembayaranSukses.class);
    ...
    private void showAlert(String title, String message) {
        new AlertDialog.Builder(MultifinancePembayaran.this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
...
//                edIdPelNo.setText("");
    ...
    private void showConfirmPinMultifinancePasca(String msg) {
        final Dialog dialog = new Dialog(MultifinancePembayaran.this);
        finis = false;
        sdialog = true;
            ...
            public void onClick(View v) {
                ...
                if (jwtlocal.equals("0") || timeup)
                    ...
                    if (Utility.is6digit(strPIN)) {
                        new AsyncPaymentMultifinancePascaBMT().execute();
                    } else {
    ...
    protected void onResume() {
        ...
        if (stats && sts > 1) {
            ...
            AuthLoginPPOB task = new AuthLoginPPOB(MultifinancePembayaran.this, MultifinancePembayaran.this);
            task.execute();
        }
    }
