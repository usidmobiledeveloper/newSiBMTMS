...
import com.usid.mobilebmt.mandirisejahtera.databinding.ActivityMultifinancePembayaranSuksesBinding;
...
public class MultifinancePembayaranSukses extends AppCompatActivity {
    ...
    ActivityMultifinancePembayaranSuksesBinding binding;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding=ActivityMultifinancePembayaranSuksesBinding.inflate(getLayoutInflater());
        ...
        Utility.playNotificationSound(MultifinancePembayaranSukses.this);
        SnHp telpMan = new SnHp(MultifinancePembayaranSukses.this);
        if (5 != telpMan.telephonyManager().getSimState()) {
            strImsi = "TIDAK ADA KARTU";
        } else {

                strImsi = Utility.getIMSIRead(MultifinancePembayaranSukses.this);

        }
        if (strImsi.equals("TIDAK ADA KARTU")) {
            showExt("ERROR", "Masukkan SIM CARD!");
        } else {
            tvInfo.setText("Transaksi Pembayaran " + strJudul + " Berhasil");
            ...
            Utility.addRow(arr, MultifinancePembayaranSukses.this, mKonten);
    ...
    private void takeScreenshot() {
        try {
            ...
            Toast.makeText(MultifinancePembayaranSukses.this, "Gagal menyimpan screenshoot", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
    ...
    private void showExt(String title, String message) {
        new AlertDialog.Builder(MultifinancePembayaranSukses.this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
...
//        Uri uri = Uri.parse("file://" + imageFile.getAbsolutePath());
        Uri uri = FileProvider.getUriForFile(MultifinancePembayaranSukses.this, BuildConfig.APPLICATION_ID + ".provider", imageFile);
