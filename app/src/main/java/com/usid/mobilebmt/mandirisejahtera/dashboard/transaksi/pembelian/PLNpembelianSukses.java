...
import com.usid.mobilebmt.mandirisejahtera.databinding.ActivityPlnpraSuksesBinding;
...
public class PLNpembelianSukses extends AppCompatActivity {
    private TextView tvInfo;
    private Button btnOK;
    private Bundle extras;
    private LinearLayout mKonten;
    private String strImsi = "", strIsi = "", tokenPLN = "";
    private ScrollView scrollView;

    ActivityPlnpraSuksesBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding=ActivityPlnpraSuksesBinding.inflate(getLayoutInflater());
        ...
        Utility.playNotificationSound(PLNpembelianSukses.this);
        SnHp telpMan = new SnHp(PLNpembelianSukses.this);
        if (5 != telpMan.telephonyManager().getSimState()) {
            strImsi = "TIDAK ADA KARTU";
        } else {

            strImsi = Utility.getIMSIRead(PLNpembelianSukses.this);
        }
        if (strImsi.equals("TIDAK ADA KARTU")) {
            ...
            Utility.addRow(arr, PLNpembelianSukses.this, mKonten);

            tokenPLN = arr.get(arr.size() - 1);
            tokenPLN = tokenPLN.replace("TOKEN", "").replace(":", "").replace(" ", "");

            tvInfo.setText("Pembelian Token Listrik Berhasil");
    ...
    private void takeScreenshot() {
        try {
           ...
           /* } else {
            ...
            Toast.makeText(PLNpembelianSukses.this, "Gagal menyimpan screenshoot", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
    ...
    private void showExt(String title, String message) {
        new AlertDialog.Builder(PLNpembelianSukses.this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
...
//        Uri uri = Uri.parse("file://" + imageFile.getAbsolutePath());
        Uri uri = FileProvider.getUriForFile(PLNpembelianSukses.this, BuildConfig.APPLICATION_ID + ".provider", imageFile);
