...
import com.usid.mobilebmt.mandirisejahtera.databinding.ActivityPlnpascaSuksesBinding;
...
public class PLNpembayaranSukses extends AppCompatActivity {
    ...
    ActivityPlnpascaSuksesBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityPlnpascaSuksesBinding.inflate(getLayoutInflater());
        ...
        Utility.playNotificationSound(PLNpembayaranSukses.this);
        SnHp telpMan = new SnHp(PLNpembayaranSukses.this);
        if (5 != telpMan.telephonyManager().getSimState()) {
            strImsi = "TIDAK ADA KARTU";
        } else {

            strImsi = Utility.getIMSIRead(PLNpembayaranSukses.this);
        ...
        if (strImsi.equals("TIDAK ADA KARTU")) {
            ...
            Utility.addRow(arr, PLNpembayaranSukses.this, mKonten);
            tvInfo.setText("Transaksi Pembayaran Listrik Berhasil");
    ...
    private void takeScreenshot() {
        try {
           ...
           /* } else {
            ...
            Toast.makeText(PLNpembayaranSukses.this, "Gagal menyimpan screenshoot", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
    ...
    private void showExt(String title, String message) {
        new AlertDialog.Builder(PLNpembayaranSukses.this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
...
//        Uri uri = Uri.parse("file://" + imageFile.getAbsolutePath());
        Uri uri = FileProvider.getUriForFile(PLNpembayaranSukses.this, BuildConfig.APPLICATION_ID + ".provider", imageFile);
