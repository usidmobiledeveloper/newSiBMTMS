...
public class ETollPembelian extends AppCompatActivity implements IAsyncHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_etoll_pembelian);

        cnd = new CtdPPOB(ETollPembelian.this);
        ...
        SnHp telpMan = new SnHp(ETollPembelian.this);
        ...
        btnOK.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (jwtlocal.equals("0")) {
                    Toast.makeText(ETollPembelian.this, "Tidak bisa diproses, periksa koneksi anda tunggu hingga indikator berwarna biru", Toast.LENGTH_SHORT).show();
                } else {
                    if (edOperator.getText().toString().equals("") || edNominal.getText().toString().equals(""))
                        ...
                        showAlert("KESALAHAN", "Nomor Kartu tidak boleh kosong!");
                    /*else if (edNoHp.getText().toString().length() < 10 || edNoHp.getText().toString().length() > 15)
                        showAlert("KESALAHAN", "Nomor Handphone salah!");*/
                    else if (edNoHp.getText().toString().length() != 16)
                        showAlert("KESALAHAN", "Nomor Kartu salah!");
                    else {
                        finis = false;
...
//                        if (strNamaOperator.equals(edOperator.getText().toString().trim())) {
        ...
        btnDaftarTrf.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(ETollPembelian.this, DaftarEToll.class);
                startActivityForResult(intent, 0);
            }
        });
        pdLoadingInq = new ProgressDialog(ETollPembelian.this);
        ...
        if (strImsi.equals("TIDAK ADA KARTU")) {
            ...
            new AsyncInqListEMOney().execute();
        }
    ...
    private class AsyncInqListEMOney extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL objs = new URL(MyVal.URL_BASE_PPOB() + new NumSky(ETollPembelian.this).decrypt(getResources().getString(R.string.urlListProdEToll)));
                Log.d("AYIK", "etoll url "+ objs.toString());
    ...
    private class AsyncListProduk extends AsyncTask<Void, Void, Void> {
        ProgressDialog pdLoading = new ProgressDialog(ETollPembelian.this);
        ...
        protected void onPreExecute() {
            ...
            //Toast.makeText(ETollPembelian.this, ""+ kodeproduk, Toast.LENGTH_SHORT).show();
        }

        @Override
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE() + new NumSky(ETollPembelian.this).decrypt(getResources().getString(R.string.urllistproduk)));
        ...
        protected void onPostExecute(Void result) {
            ...
            if (stInfoMutasi) {
                ListAdapter adapter = new SimpleAdapter(ETollPembelian.this, oslist, R.layout.list_item, new String[]{"nmproduk"}, new int[]{R.id.text1});
                AlertDialog.Builder alertDialog = new AlertDialog.Builder(ETollPembelian.this);
                list = new ListView(ETollPembelian.this);
    ...
    private class AsyncInqBMT extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE() + new NumSky(ETollPembelian.this).decrypt(getResources().getString(R.string.urlinquirypulsa)));
    ...
    private class AsyncINQPPOB extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE_PPOB() + new NumSky(ETollPembelian.this).decrypt(getResources().getString(R.string.urlCekStatusProd)));
    ...
    private class AsyncPaymentEMoneyBMT extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                URL obj = new URL(MyVal.URL_BASE_PPOB() + new NumSky(ETollPembelian.this).decrypt(getResources().getString(R.string.urlEToll)));
...
//                    String keta = "Transaksi Pembelian Pulsa Berhasil" + "\nTanggal    : " + strDate + "\nProduk     : " + layanan + "\nNomor HP.  : " + strIDPEL + "\nNominal    : " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(nominal), 15) + "\nBiaya Admin: " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(biayaAdmin), 15) + "\nTotal Bayar: " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(jumlah), 15) + "\nSN " + struk + "\n";
                    String keta = "Transaksi Pembelian E-Toll Berhasil" + "\nTanggal    : " + strDate + "\nFaktur     : " + strFaktur + "\nTrxID      : " + trxID + "\nDari       : " + strRekAsal + "\nProduk     : " + layanan + "\nNomor Kartu  : " + strIDPEL + "\nNominal    : " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(nominal), 15) + "\nBiaya Admin: " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(biayaAdmin), 15) + "\nTotal Bayar: " + Utility.leftRight("Rp.", Utility.DoubleToCurrency(jumlah), 15) + /*"\nSN " + struk +*/ "\n";
                    SysDB dbsys = new SysDB(ETollPembelian.this);
                    dbsys.open();
...
//                    dbsys.insertSys(strTgl, "Pembelian Pulsa Seluler (Reff)\n" + keta);
                    dbsys.insertSys(strTgl, "Pembelian E-Toll SUKSES (Reff)\n" + keta);
                    dbsys.close();
                    ...
                    dbsys.open();
                    dbsys.CreateTableSys();
                    dbsys.insertSys(strTgl, "Pembelian E-Toll STATUS " + msga);
                    dbsys.close();
                    sdialog = true;
                    showExt("Pembelian E-Toll STATUS", msga);
                }
            } else {
                if (ket.startsWith("404")) {
                    ...
                    dbsys.open();
                    dbsys.CreateTableSys();
                    dbsys.insertSys(strTgl, "Pembelian E-Toll TIMEOUT " + msga);
                    dbsys.close();
                    sdialog = true;
                    showExt("Pembelian E-Toll TIMEOUT", msga);
                } else {
                    String msga = "#" + ket + "\n";
                    SysDB dbsys = new SysDB(ETollPembelian.this);
                    dbsys.open();
                    dbsys.CreateTableSys();
                    dbsys.insertSys(strTgl, "Pembelian E-Toll GAGAL " + msga);
                    dbsys.close();
                    sdialog = true;
                    showExt("Pembelian E-Toll GAGAL", msga);
                }
    ...
    private void AlertDialogViewOperator() {
        AlertDialog.Builder builder = new AlertDialog.Builder(ETollPembelian.this);
    ...
    private void showExt(String title, String message) {
        new AlertDialog.Builder(ETollPembelian.this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                finish();
                dialog.dismiss();
    ...
    private void showConfirmBeli(String Judul, final String prod, final String nohp, String nomi, String admin, String jumlah) {
        final Dialog dialog = new Dialog(ETollPembelian.this);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.custom_dialog_confirm_trxetoll);
        ...
        SysDB dbsys = new SysDB(ETollPembelian.this);
        dbsys.open();
        dbsys.CreateTableEToll();
        int jumla = 0;
        Cursor cur1 = dbsys.cekETollData(edNoHp.getText().toString());
        if (cur1.moveToFirst()) {
            jumla = cur1.getInt(0);
        }
        cur1.close();
                ...
                if (jwtlocal.equals("0"))
                    ...
                    if (cbsimpan.isChecked()) {
                        if (edNama.getText().toString().equals("")) {
                            Toast.makeText(ETollPembelian.this, "Tidak bisa diproses, Nama tidak boleh kosong", Toast.LENGTH_SHORT).show();
                        } else {
                            try {
                                SysDB dbsys = new SysDB(ETollPembelian.this);
                                dbsys.open();
                                String info = edNama.getText().toString().trim();
                                dbsys.insertEToll(edNoHp.getText().toString(), info);
                                dbsys.close();
            ...
            public void onClick(View v) {
                ...
                if (jwtlocal.equals("0") || timeup)
                    ...
                    if (Utility.is6digit(strPIN)) {
                        new AsyncPaymentEMoneyBMT().execute();
                    } else {
    ...
    private void showConfirmBeliSukses(String Judul, String tgls, String prod, String nohp, String nomi, String admin, String jumlah, String sn) {
        ...
        Intent antarbank = new Intent(ETollPembelian.this, PembelianSukses.class);

        ArrayList<String> arr = new ArrayList<String>();

        arr.add("Tanggal#" + tgls);
        arr.add("Produk#" + prod);
        arr.add("No. Kartu#" + nohp);
        ...
        antarbank.putExtra("judul", "Etoll Pembelian Berhasil");
        ...
        /*final Dialog dialog = new Dialog(ETollPembelian.this);
        Utility.playNotificationSound(ETollPembelian.this);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.custom_dialog_confirm_trxetollsukses);
        TextView textPin = (TextView) dialog.findViewById(R.id.textPin);
        textPin.setText(Judul);
...
//            View v1 = getWindow().getDecorView().getRootView();
            ...
            Toast.makeText(ETollPembelian.this, "Gagal menyimpan screenshoot", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
    ...
    protected void onResume() {
        ...
        if (stats && sts > 1) {
            ...
            AuthLoginPPOB task = new AuthLoginPPOB(ETollPembelian.this, ETollPembelian.this);
            task.execute();
        }
    }
