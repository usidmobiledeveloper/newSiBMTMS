...
public class DaftarEToll extends AppCompatActivity {
    private SysDB dbsys;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbsys = new SysDB(DaftarEToll.this);
        dbsys.open();
        dbsys.CreateTableEToll();
        int jumla = 0;
        Cursor cur1 = dbsys.cekETollData();
        if (cur1.moveToFirst()) {
            jumla = cur1.getInt(0);
        }
        cur1.close();
        ...
        rekening.clear();
        nama.clear();
        Cursor cur = dbsys.daftarEToll();
        if (cur.moveToFirst()) {
            do {
                HashMap<String, String> temp = new HashMap<String, String>();
                temp.put("oprt", "Nama : " + cur.getString(1));
                temp.put("nohp", "No. Kartu : " + cur.getString(0));
                list.add(temp);
    ...
    private void showAlert(String title, String message, final String rek) {
        new AlertDialog.Builder(this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("YA", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                SysDB dbsys = new SysDB(DaftarEToll.this);
                dbsys.open();
                dbsys.DeleteEToll(rek);
                dbsys.close();
                finish();
                dialog.dismiss();
            }
