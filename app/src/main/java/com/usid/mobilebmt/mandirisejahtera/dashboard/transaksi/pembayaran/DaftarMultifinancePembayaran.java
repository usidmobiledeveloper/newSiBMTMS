...
public class DaftarMultifinancePembayaran extends AppCompatActivity {
    private SysDB dbsys;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbsys = new SysDB(DaftarMultifinancePembayaran.this);
        dbsys.open();
        dbsys.CreateTableMultifinance();
        int jumla = 0;
        Cursor cur1 = dbsys.cekMultifinanceData();
        if (cur1.moveToFirst()) {
            jumla = cur1.getInt(0);
        }
        cur1.close();
        ...
        SimpleAdapter adapter = new SimpleAdapter(this, list, R.layout.custom_row_view, new String[]{"nama", "idpel"}, new int[]{R.id.text1, R.id.text2});
        lv.setAdapter(adapter);
        list.clear();
        rekening.clear();
        nama.clear();
        Cursor cur = dbsys.daftarMultifinance();
        if (cur.moveToFirst()) {
            do {
                HashMap<String, String> temp = new HashMap<String, String>();
                temp.put("nama", "Nama : " + cur.getString(1));
                temp.put("idpel", "ID Pel. : " + cur.getString(0));
                list.add(temp);
        ...
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> av, View v, int position, long id) {
                ...
                intent.putExtra("IDPEL", strnorek);
    ...
    private void showAlert(String title, String message, final String rek) {
        new AlertDialog.Builder(this).setIconAttribute(android.R.attr.alertDialogIcon).setTitle(title).setMessage(message).setCancelable(false).setPositiveButton("YA", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                SysDB dbsys = new SysDB(DaftarMultifinancePembayaran.this);
                dbsys.open();
                dbsys.DeleteMultifinance(rek);
                dbsys.close();
                finish();
                dialog.dismiss();
            }
